"""
Mission Control Voice Agent
===========================
Real-time voice assistant that connects to the MCP server and exposes a
polished operator experience for world briefings and system actions.

Run:
  uv run mission_control_voice             – LiveKit Cloud mode
  uv run mission_control_agent.py console  – text-only console mode
"""
...
STT_PROVIDER       = _provider_setting("STT_PROVIDER", "sarvam")
LLM_PROVIDER       = _provider_setting("LLM_PROVIDER", "gemini")
TTS_PROVIDER       = _provider_setting("TTS_PROVIDER", "openai")
...
logger = logging.getLogger("mission-control-agent")
...
def _require_env(var_name: str, provider_name: str) -> str:
    ...
    if not value:
        raise ValueError(
            f"{provider_name} requires {var_name}. "
            f"Set it in {PROJECT_DIR / '.env'} or export it before starting the agent."
        )
    return value
...
def _load_google_plugin():
    from livekit.plugins import google as lk_google

    return lk_google


def _load_groq_plugin():
    from livekit.plugins import groq as lk_groq

    return lk_groq


def _load_sarvam_plugin():
    from livekit.plugins import sarvam

    return sarvam


def _load_silero_plugin():
    ...
    if STT_PROVIDER == "sarvam":
        if not os.getenv("SARVAM_API_KEY", "").strip():
            if os.getenv("OPENAI_API_KEY", "").strip():
                logger.warning(
                    "SARVAM_API_KEY is missing in %s; falling back to OpenAI Whisper STT.",
                    PROJECT_DIR / ".env",
                )
                lk_openai = _load_openai_plugin()
                return lk_openai.STT(model="whisper-1")
            raise ValueError(
                f"Sarvam STT requires SARVAM_API_KEY. Set it in {PROJECT_DIR / '.env'} "
                "or provide OPENAI_API_KEY to fall back to Whisper."
            )
        logger.info("STT → Sarvam Saaras v3")
        sarvam = _load_sarvam_plugin()
        return sarvam.STT(
            language="unknown",
            model="saaras:v3",
            mode="transcribe",
        ...
        _require_env("GROQ_API_KEY", "Groq STT")
        lk_groq = _load_groq_plugin()
...
def _build_llm():
    if LLM_PROVIDER == "openai":
        ...
        _require_env("OPENAI_API_KEY", "OpenAI LLM")
        lk_openai = _load_openai_plugin()
        return lk_openai.LLM(model=OPENAI_LLM_MODEL)
    elif LLM_PROVIDER == "gemini":
        logger.info("LLM → Google Gemini (%s)", GEMINI_LLM_MODEL)
        lk_google = _load_google_plugin()
        return lk_google.LLM(
            model=GEMINI_LLM_MODEL,
            api_key=_require_env("GOOGLE_API_KEY", "Google Gemini"),
        )
    elif LLM_PROVIDER == "groq":
        logger.info("LLM → Groq (%s)", GROQ_LLM_MODEL)
        _require_env("GROQ_API_KEY", "Groq LLM")
        lk_groq = _load_groq_plugin()
...
def _build_tts():
    if TTS_PROVIDER == "sarvam":
        _require_env("SARVAM_API_KEY", "Sarvam TTS")
        ...
        _require_env("OPENAI_API_KEY", "OpenAI TTS")
        logger.info("TTS → OpenAI TTS (%s / %s)", OPENAI_TTS_MODEL, OPENAI_TTS_VOICE)
        lk_openai = _load_openai_plugin()
        return lk_openai.TTS(model=OPENAI_TTS_MODEL, voice=OPENAI_TTS_VOICE, speed=TTS_SPEED)
    elif TTS_PROVIDER == "groq":
        _require_env("GROQ_API_KEY", "Groq TTS")
        logger.info("TTS → Groq TTS (%s / %s)", GROQ_TTS_MODEL, GROQ_TTS_VOICE)
        lk_groq = _load_groq_plugin()
        return lk_groq.TTS(model=GROQ_TTS_MODEL, voice=GROQ_TTS_VOICE)
    else:
...
class MissionControlAgent(Agent):
    """
...
async def entrypoint(ctx: JobContext) -> None:
    logger.info(
        "Mission Control online – room: %s | STT=%s | LLM=%s | TTS=%s",
    ...
    await session.start(
        agent=MissionControlAgent(stt=stt, llm=llm, tts=tts),
        room=ctx.room,
    )
